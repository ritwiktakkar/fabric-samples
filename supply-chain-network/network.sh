...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating BuyingAgent Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-buyingagent.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
    ...
    infoln "Creating Auditor Identities"
    ...
    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-fps.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
    ...
    infoln "Creating RawMaterialsSupplier Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-rm.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
    ...
    infoln "Creating TextileManufacturer Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-tx.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
    ...
    infoln "Creating BlockchainConsultant (Orderer) Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-bc.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  if [ "$CRYPTO" == "cfssl" ]; then
    ...
    peer_cert peer peer0.retailer.com retailer
    peer_cert admin Admin@retailer.com retailer

    infoln "Creating BuyingAgent Identities"
    #function_name cert-type   CN   org
    peer_cert peer peer0.buyingagent.com buyingagent
    peer_cert admin Admin@buyingagent.com buyingagent

    infoln "Creating Auditor Identities"
    #function_name cert-type   CN   org
    peer_cert peer peer0.auditor.com auditor
    peer_cert admin Admin@auditor.com auditor

    infoln "Creating FullPackageSupplier Identities"
    #function_name cert-type   CN   org
    peer_cert peer peer0.fullpackagesupplier.com fullpackagesupplier
    peer_cert admin Admin@fullpackagesupplier.com fullpackagesupplier
  
    infoln "Creating RawMaterialsSupplier Identities"
    #function_name cert-type   CN   org
    peer_cert peer peer0.rawmaterials.com rawmaterialssupplier
    peer_cert admin Admin@rawmaterials.com rawmaterialssupplier

    infoln "Creating TextileManufacturer Identities"
    #function_name cert-type   CN   org
    peer_cert peer peer0.textilemanufacturer.com textilemanufacturer
    peer_cert admin Admin@textilemanufacturer.com textilemanufacturer

    infoln "Creating Orderer Org Identities"
    #function_name cert-type   CN   
    orderer_cert orderer orderer.blockchainconsultant.com
    orderer_cert admin Admin@.blockchainconsultant.com
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/retailer/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
  ...
  infoln "Generating CCP files for Retailer, BuyingAgent, Auditor, TextileManufacturer, FullPackageSupplier, RawMaterialsSupplier and BlockchainConsultant"
...
# call the script to create the channel, join the peers of retailer and buyingagent,
...
function networkDown() {
  ...
  # stop fps containers also in addition to retailer and buyingagent, in case we were running sample to add fps
  COMPOSE_FPS_BASE_FILES="-f addFPS/compose/${COMPOSE_FILE_FPS_BASE} -f addFPS/compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_FPS_BASE}"
  COMPOSE_FPS_COUCH_FILES="-f addFPS/compose/${COMPOSE_FILE_FPS_COUCH} -f addFPS/compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_FPS_COUCH}"
  COMPOSE_FPS_CA_FILES="-f addFPS/compose/${COMPOSE_FILE_FPS_CA} -f addFPS/compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_FPS_CA}"
  COMPOSE_FPS_FILES="${COMPOSE_FPS_BASE_FILES} ${COMPOSE_FPS_COUCH_FILES} ${COMPOSE_FPS_CA_FILES}"

  if [ "${CONTAINER_CLI}" == "docker" ]; then
    DOCKER_SOCK=$DOCKER_SOCK ${CONTAINER_CLI_COMPOSE} ${COMPOSE_FILES} ${COMPOSE_FPS_FILES} down --volumes --remove-orphans
  elif [ "${CONTAINER_CLI}" == "podman" ]; then
    ${CONTAINER_CLI_COMPOSE} ${COMPOSE_FILES} ${COMPOSE_FPS_FILES} down --volumes
  else
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.blockchainconsultant.com docker_peer0.retailer.com docker_peer0.buyingagent.com
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/buyingagent/msp organizations/fabric-ca/buyingagent/tls-cert.pem organizations/fabric-ca/buyingagent/ca-cert.pem organizations/fabric-ca/buyingagent/IssuerPublicKey organizations/fabric-ca/buyingagent/IssuerRevocationPublicKey organizations/fabric-ca/buyingagent/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/ordererOrg/msp organizations/fabric-ca/ordererOrg/tls-cert.pem organizations/fabric-ca/ordererOrg/ca-cert.pem organizations/fabric-ca/ordererOrg/IssuerPublicKey organizations/fabric-ca/ordererOrg/IssuerRevocationPublicKey organizations/fabric-ca/ordererOrg/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf addFPS/fabric-ca/fps/msp addFPS/fabric-ca/fps/tls-cert.pem addFPS/fabric-ca/fps/ca-cert.pem addFPS/fabric-ca/fps/IssuerPublicKey addFPS/fabric-ca/fps/IssuerRevocationPublicKey addFPS/fabric-ca/fps/fabric-ca-server.db'
...
# use this as the default docker-compose yaml definition for fps
COMPOSE_FILE_FPS_BASE=compose-fps.yaml
# use this as the docker compose couch file for fps
COMPOSE_FILE_FPS_COUCH=compose-couch-fps.yaml
# certificate authorities compose file
COMPOSE_FILE_FPS_CA=compose-ca-fps.yaml
#
