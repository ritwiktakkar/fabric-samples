...
# It joins the fps peers to the channel previously setup in
...
set -x
peer channel fetch 0 $BLOCKFILE -o localhost:7050 --ordererTLSHostnameOverride orderer.blockchainconsultant.com -c $CHANNEL_NAME --tls --cafile "$ORDERER_CA" >&log.txt
res=$?
{ set +x; } 2>/dev/null
cat log.txt
verifyResult $res "Fetching config block from orderer has failed"

infoln "Joining fps peer to the channel..."
joinChannel 3

infoln "Setting anchor peer for fps..."
setAnchorPeer 3

successln "Channel '$CHANNEL_NAME' joined"
successln "FPS peer successfully added to network"
