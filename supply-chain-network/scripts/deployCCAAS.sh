...
startDockerContainer() {
  ...
  if [ "$CCAAS_DOCKER_RUN" = "true" ]; then
    ...
    set -x
    ${CONTAINER_CLI} run --rm -d --name peer0retailer_${CC_NAME}_ccaas  \
                  --network supply_chain_network \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest

    ${CONTAINER_CLI} run  --rm -d --name peer0buyingagent_${CC_NAME}_ccaas \
                  --network supply_chain_network \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest
    ...
    infoln "    ${CONTAINER_CLI} run --rm -d --name peer0retailer_${CC_NAME}_ccaas  \
                  --network supply_chain_network \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest"
    infoln "    ${CONTAINER_CLI} run --rm -d --name peer0buyingagent_${CC_NAME}_ccaas  \
                  --network supply_chain_network \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest"
...
## Install chaincode on peer0.retailer and peer0.buyingagent
infoln "Installing chaincode on peer0.retailer..."
installChaincode 1
infoln "Install chaincode on peer0.buyingagent..."
installChaincode 2
...
## approve the definition for retailer
approveForMyOrg 1

## check whether the chaincode definition is ready to be committed
## expect retailer to have approved and buyingagent not to
checkCommitReadiness 1 "\"RetailerMSP\": true" "\"BuyingAgentMSP\": false"
checkCommitReadiness 2 "\"RetailerMSP\": true" "\"BuyingAgentMSP\": false"

## now approve also for buyingagent
approveForMyOrg 2

## check whether the chaincode definition is ready to be committed
## expect them both to have approved
checkCommitReadiness 1 "\"RetailerMSP\": true" "\"BuyingAgentMSP\": true"
checkCommitReadiness 2 "\"RetailerMSP\": true" "\"BuyingAgentMSP\": true"
