...
function peer_cert() {
    ...
    mkdir -p "organizations/peerOrganizations/$ORG..com/ca"
    mkdir -p "organizations/peerOrganizations/$ORG..com/msp/cacerts"
    mkdir -p "organizations/peerOrganizations/$ORG..com/msp/tlscacerts"
    mkdir -p "organizations/peerOrganizations/$ORG..com/peers"
    mkdir -p "organizations/peerOrganizations/$ORG..com/tlsca"

    CERT_DIR=organizations/peerOrganizations/$ORG..com

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-peer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.$ORG..com-cert.pem"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/ca/ca.$ORG..com-cert.pem"
...
function orderer_cert() {
    TYPE=$1 #orderer user
    USER=$2 #orderer.blockchainconsultant.com

    mkdir -p organizations/ordererOrganizations/.com/ca
    mkdir -p organizations/ordererOrganizations/.com/msp/cacerts
    mkdir -p organizations/ordererOrganizations/.com/msp/tlscacerts
    mkdir -p organizations/ordererOrganizations/.com/orderers
    mkdir -p organizations/ordererOrganizations/.com/tlsca

    CERT_DIR=organizations/ordererOrganizations/.com

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-orderer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca..com-cert.pem"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/cacerts/"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/tlscacerts/tlsca..com-cert.pem"
...
function generate_orderer_certs() {
    ...
    for DIR in cacerts keystore signcerts tlscacerts; do
        mkdir -p "organizations/ordererOrganizations/.com/orderers/$USER/msp/$DIR"
    done

    mkdir -p "organizations/ordererOrganizations/.com/orderers/$USER/tls"
    ...
    cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/orderers/$USER/msp/tlscacerts/tlsca..com-cert.pem"
